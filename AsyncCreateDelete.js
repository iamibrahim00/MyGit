    ...
    return new Promise((resolve,reject) =>{
        ...
        intervalId =setInterval(() =>{
         ...
         posts.forEach((post, index) =>{
            resolve( output += `<li>${post.title} last updated ${(new Date().getTime() - post.createdAt)/1000} seconds ago </li> `)
...
function createPost(post){
    return new Promise((resolve,reject)=>{
        setTimeout(() => {
            ...
            if(!error){
                resolve({...post,createdAt:new Date().getTime()})
            }
        }, 2000);
